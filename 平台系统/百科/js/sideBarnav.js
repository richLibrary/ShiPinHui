$(document).ready(function(){
    ...
    allEle.each(function(i){
        ...
        if($(this).hasClass('headlineOne')){
            sideAnchor = sideIndex = $(this).find('.anchorOne').eq(0).attr('name');
            ddClass = 'sideCatalog-item1';
        }else{
            sideAnchor = $(this).find('.anchorTwo').eq(0).attr('name');
    ...
    //设置导航的位置
    var slideTop = $(window).height() - $('.slide').height()+100;
